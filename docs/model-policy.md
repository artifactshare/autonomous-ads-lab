...
| Role | Model | 理由 | 使いどころ |
|---|---|---|---|
| hypothesis | **fable-5** | メタ認知が強く前提を疑える。weekly limitが厳しいので週3回まで（超過分はopusへフォールバック） | 仮説の生成、ブラッシュアップ、検証設計（weekly learning loop） |
| analysis | **opus-5** | 計算・ロジックが強い | metrics解釈、prediction vs actual、予算再配分、統計判断 |
| evaluation | **sonnet-5** | 頻度が高く（creative毎）、rubric採点は言語判断中心 | VLM採点、hard constraints判定 |
| copywriting | **sonnet-5** | 言葉が上手で安い | 広告コピー、hook/CTAバリアント |
| narrative | **sonnet-5** | 同上 | ジャーナル・レポートの文章 |
