...
- **Channel (added 9/14 evening)**: before paid reach, reply where people ask. One human reply with the real-loop video did in 7 minutes what 16 paid days did not (see P9).
...
| P9 | Replying (from @techtalkjp, with a 30s real-loop video) to posts that ask for the product converts better per dollar than paid video | testing (from 9/14) | 9/14 17:43 reply on @akshatag77's thread ("shared workspace editable by humans and agents?"): 18 impressions, 1 like — and **7 minutes later a 2-person Singapore studio signed up, shared an API doc, and the second member viewed it** (production `adopter-signals`). Attribution not yet confirmed (GA4 source pending). Cost $0. Compare: 16 paid ad-days, $23.3, 0 sign-ups. Routine: weekly `reply_candidates` needs-human issue; judge on sign-ups/first shares within 24h of each reply |
| P6 | 5-second AI-generated video (H3 Max, text burned in post) is the right format | **refuted (9/14)** → footage mode | Within-video *style* (absurdist metaphor vs literal workflow demo) is contested by experiment 3, still awaiting deploy. Video-vs-static untested (idea 3). Format spec is now 8s H3 Max Turbo per the 9/3 comparison |
| P10 | Real loop footage + a hook that names the viewer's current workaround beats generated video on landed sessions per dollar | testing (creatives 9–12 evaluated 9/14, eligible for deploy after the P8 readout on 9/21) | Pre-scores 5.0–6.0 vs 4.5 for creatives 3/8; product_clarity 6–8 vs 3. Media cost $0. Same footage, only the hook card and end line vary, so differences read as hook differences |
